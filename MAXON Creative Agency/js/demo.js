 ...
 */
{
    ...
    // Randomize letters function. Used when navigating the portfolioshow to switch the curretn portfolio´s texts.
    ...
    // Hide each of the letters with random delays. Used when showing the current portfolio´s content.
    ...
    class Portfolio {
        constructor(el) {
            ...
            this.DOM.imgWrap = this.DOM.el.querySelector('.portfolio__img-wrap');
            // The image element.
            this.DOM.img = this.DOM.imgWrap.querySelector('.portfolio__img');
            // The texts: the parent wrap, title, number and side text.
            this.DOM.texts = {
                wrap: this.DOM.el.querySelector('.portfolio__title-wrap'),
                title: this.DOM.el.querySelector('.portfolio__title'),
                number: this.DOM.el.querySelector('.portfolio__number'),
                side: this.DOM.el.querySelector('.portfolio__side'),
            };
            ...
            // We have 5 different possible positions for a portfolio: center, bottom right, top left and outside the viewport (top left or bottom right).
            this.calcTransforms();
            // Init/Bind events.
            this.initEvents();
        }
        ...
        // Gets the transforms per portfolio position.
        calcTransforms() {
            /*
            Each position corresponds to the position of a given portfolio:
            0: left top corner outside the viewport
            1: left top corner (prev portfolio position)
            2: center (current portfolio position)
            3: right bottom corner (next portfolio position)
        ...
        // Tilt the image wrap and texts when mouse moving the current portfolio.
        tilt(ev) {
        ...
        // Positions one portfolio (left, right or current) in the viewport.
        position(pos) {
        ...
        // Check if the portfolio is positioned on the right side (if it´s the next portfolio in the portfolioshow).
        isPositionedRight() {
            return this.isRight;
        }
        // Check if the portfolio is positioned on the left side (if it´s the previous portfolio in the portfolioshow).
        isPositionedLeft() {
            return this.isLeft;
        }
        // Check if the portfolio is the current one.
        isPositionedCenter() {
        ...
        reset() {
            ...
            this.DOM.el.classList = 'portfolio';
        }
        hide() {
            TweenMax.set(this.DOM.imgWrap, {x:0, y:0, rotationX:0, rotationY:0, rotationZ:0, opacity:0});
        }
        // Moves a portfolio to a specific position defined in settings.position.
        // Also, moves it from position settings.from and if we need to reset the image scale when moving the portfolio then settings.resetImageScale should be true.
        moveToPosition(settings) {
            return new Promise((resolve, reject) => {
                /*
                Moves the portfolio to a specific position:
                -2: left top corner outside the viewport
                -1: left top corner (prev portfolio position)
                0: center (current portfolio position)
                1: right bottom corner (next portfolio position)
                ...
                */
                ...
                // Reset image scale when showing the content of the current portfolio.
        ...
        // Hides the current portfolio´s texts.
        ...
        // Shows the current portfolio´s texts.
    ...
    // The Content class. Represents one content item per portfolio.
    class Content {
        constructor(el) {
            this.DOM = {el: el};
            ...
            this.DOM.backCtrl.addEventListener('click', () => portfolioshow.hideContent());
        }
        show() {
    ...
    // The portfolioshow class.
    class Portfolioshow {
        constructor(el) {
            this.DOM = {el: el};
            // The portfolios.
            this.portfolios = [];
            Array.from(this.DOM.el.querySelectorAll('.portfolio')).forEach(portfolioEl => this.portfolios.push(new Portfolio(portfolioEl)));
            // The total number of portfolios.
            this.portfoliosTotal = this.portfolios.length;
            // At least 4 portfolios to continue...
            if ( this.portfoliosTotal < 4 ) {
                return false;
            }
            // Current portfolio position.
            this.current = 0;
            this.DOM.deco = this.DOM.el.querySelector('.portfolioshow__deco');

            this.contents = [];
            Array.from(document.querySelectorAll('.content > .content__item')).forEach(contentEl => this.contents.push(new Content(contentEl)));

            // Set the current/next/previous portfolios. 
            this.render();
            this.currentportfolio.showTexts(false);
            // Init/Bind events.
            this.initEvents();
        }
        render() {
            // The current, next, and previous portfolios.
            this.currentportfolio = this.portfolios[this.current];
            this.nextportfolio = this.portfolios[this.current+1 <= this.portfoliosTotal-1 ? this.current+1 : 0];
            this.prevportfolio = this.portfolios[this.current-1 >= 0 ? this.current-1 : this.portfoliosTotal-1];
            this.currentportfolio.setCurrent();
            this.nextportfolio.setRight();
            this.prevportfolio.setLeft();
        }
        initEvents() {
            // Clicking the next and previous portfolio starts the navigation / clicking the current shows its content..
            this.clickFn = (portfolio) => {
                if ( portfolio.isPositionedRight() ) {
                    this.navigate('next');
                }
                else if ( portfolio.isPositionedLeft() ) {
                    this.navigate('prev');
                }
                else {
                    this.showContent();
            ...
            for (let portfolio of this.portfolios) {
                portfolio.DOM.imgWrap.addEventListener('click', () => this.clickFn(portfolio));
            }

            this.resizeFn = () => {
                // Reposition the portfolios.
                this.nextportfolio.setRight(this.isContentOpen);
                this.prevportfolio.setLeft(this.isContentOpen);
                this.currentportfolio.setCurrent(this.isContentOpen);
        ...
        showContent() {
            ...
            this.DOM.el.classList.add('portfolioshow--previewopen');
            ...
            // Move away right/left portfolios.
            this.prevportfolio.moveToPosition({position: -2});
            this.nextportfolio.moveToPosition({position: 2});
            // Position the current portfolio and reset its image scale value.
            this.currentportfolio.moveToPosition({position: 3, resetImageScale: true});
            // Show content and back arrow (to close the content).
            this.contents[this.current].show();
            // Hide texts.
            this.currentportfolio.hideTexts(true);
        }
        hideContent() {
            if ( !this.isContentOpen || this.isAnimating ) return;

            this.DOM.el.classList.remove('portfolioshow--previewopen');
            ...
            // Move in right/left portfolios.
            this.prevportfolio.moveToPosition({position: -1});
            this.nextportfolio.moveToPosition({position: 1});
            // Position the current portfolio.
            this.currentportfolio.moveToPosition({position: 0}).then(() => {
                allowTilt = true;
                this.isContentOpen = false;
            });
            // Show texts.
            this.currentportfolio.showTexts();
        }
        // Animates the element behind the current portfolio.
        ...
        navigate(direction) {
            ...
            const upcomingPos = direction === 'next' ? 
                    this.current < this.portfoliosTotal-2 ? this.current+2 : Math.abs(this.portfoliosTotal-2-this.current):
                    this.current >= 2 ? this.current-2 : Math.abs(this.portfoliosTotal-2+this.current);
            
            this.upcomingportfolio = this.portfolios[upcomingPos];

            // Update current.
            this.current = direction === 'next' ? 
                    this.current < this.portfoliosTotal-1 ? this.current+1 : 0 :
                    this.current > 0 ? this.current-1 : this.portfoliosTotal-1;
            
            // Move portfolios (the previous, current, next and upcoming portfolio).
            this.prevportfolio.moveToPosition({position: direction === 'next' ? -2 : 0, delay: direction === 'next' ? 0 : 0.14}).then(() => {
                if ( direction === 'next' ) {
                    this.prevportfolio.hide();
                }
            });
            
            this.currentportfolio.moveToPosition({position: direction === 'next' ? -1 : 1, delay: 0.07 });
            this.currentportfolio.hideTexts();
            
            this.bounceDeco(direction, 0.07);
            
            this.nextportfolio.moveToPosition({position: direction === 'next' ? 0 : 2, delay: direction === 'next' ? 0.14 : 0 }).then(() => {
                if ( direction === 'prev' ) {
                    this.nextportfolio.hide();
                }
            });

            if ( direction === 'next' ) {
                this.nextportfolio.showTexts();
            }
            else {
                this.prevportfolio.showTexts();
            }
            
            this.upcomingportfolio.moveToPosition({position: direction === 'next' ? 1 : -1, from: direction === 'next' ? 2 : -2, delay: 0.21 }).then(() => {
                // Reset classes.
                [this.nextportfolio,this.currentportfolio,this.prevportfolio].forEach(portfolio => portfolio.reset());
                this.render();
                allowTilt = true;
    ...
    // Init portfolioshow.
    const portfolioshow = new Portfolioshow(document.querySelector('.portfolioshow'));
    
    // Preload all the images in the page..
    const loader = document.querySelector('.loader');
    imagesLoaded(document.querySelectorAll('.portfolio__img'), {background: true}, () => document.body.classList.remove('loading'));
}
